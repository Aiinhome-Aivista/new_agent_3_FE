  ...
  return (
    <React.Fragment>
      <tr onClick={() => setExpanded(!expanded)} className="cursor-pointer hover:bg-gray-50 transition-colors">
        ...
        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{m.total_risks}</td>
                    ...
                    <div key={plan.plan_id} className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                      ...
                      {plan.risks && plan.risks.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
                          {plan.risks.map(risk => (
                            <div key={risk.id} className={`rounded-lg shadow-sm border p-3 ${getSeverityColor(risk.severity)} bg-opacity-30`}>
                                <div className="flex justify-between items-start mb-2">
                                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide bg-white shadow-sm`}>
                                        {risk.severity}
                                    </span>
                                </div>
                                ...
                                <div className="flex justify-between items-center border-t pt-2 border-black border-opacity-5">
                                    <span className="text-[10px] font-semibold capitalize opacity-75">
                                        Status: {risk.status}
                                    </span>
                                </div>
                            </div>
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col justify-center items-center">
          ...
          <span className="text-4xl font-bold text-red-600">{total_risks}</span>
        </div>
